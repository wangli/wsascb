package com.wsascb.movieShow{
	...
	public class ShowOnlyDisplay extends EventDispatcher {
		...
		private var vCenter:Boolean=false;//是否垂直居中
		...
		private var easeStr:Function=Circ.easeInOut;
		public function ShowOnlyDisplay(_mc:Sprite, str:String, strTime:Number=5000, _leftBtn:InteractiveObject=null, _rightBtn:InteractiveObject=null, _w:Number=100, _h:Number=100):void {
			bigpicArr = new Array();
			...
			timer=new Timer(strTime);
			timer.addEventListener(TimerEvent.TIMER,intervalFun);
		}
		...
		private function fillData():void {
			...
			loadAsset(mcTemp,bigpicArr[mcSub]);
			sedIvl();
		}
		//自动播放
		...
		//间隔时间事件命令,自动切换资源
		private function intervalFun(evt:TimerEvent):void {
			//允许加载时在加载
			if (loadState == true) {
				gotoPlayMc();
			}
		...
		private function exchangeMc(_val:Number):void {
			...
			//加载图片到深度高的对象 
			loadAsset(mcTemp,bigpicArr[_val]);
		}
		//替换复制数据
			...
			switch (exef) {
				...
				case "RollMoveLeft" :
					...
					if(_val.width<imgWidth){
						TweenLite.to(_val, exchangeMcTime, {x:(imgWidth-_val.width)/2, ease:easeStr});
						}else{
						TweenLite.to(_val, exchangeMcTime, {x:0, ease:easeStr});
					}
					if(_val.height<imgHeight&&vCenter){
						_val.y=(imgHeight-_val.height)/2
					}else{
						_val.y=0;
					}
					if (_val.name == "downMc") {
						TweenLite.to(upMc, exchangeMcTime, {x:(imgWidth + exefMcSpace), ease:easeStr});
						} else {
						TweenLite.to(downMc, exchangeMcTime, {x:(imgWidth + exefMcSpace), ease:easeStr});
					}
					break;
				case "RollMoveRight" :
					_val.x = _val.width + exefMcSpace;
					if(_val.width<imgWidth&&vCenter){
						TweenLite.to(_val, exchangeMcTime, {x:(imgWidth-_val.width)/2, ease:easeStr});
						}else{
						TweenLite.to(_val, exchangeMcTime, {x:0, ease:easeStr});
					}
					...
					if (_val.name == "downMc") {
						TweenLite.to(upMc, exchangeMcTime, {x:(-upMc.width-exefMcSpace), ease:easeStr});
						} else {
						TweenLite.to(downMc, exchangeMcTime, {x:(-downMc.width-exefMcSpace), ease:easeStr});
					}
					break;
			}
		}
		...
		//是否填满
		public function set zoomfill(_val:Boolean):void{
			this.zoomFill=_val;
		}
		//是否全部显示
		public function set zoombound(_val:Boolean):void{
			this.zoomBound=_val;
		}
	}
}